...
fn handle_profiles(window: &mut MainWindow, message: Message) -> Task<Message> {
    match message {
                ...
                Err(e) if e != "Cancelled" => {
                    ...
                    window.set_status(format!("File error: {}", e), StatusSeverity::Error)
                }
                _ => Task::none(),
            },
                ...
                Ok(name) => {
                    ...
                    window.set_status(format!("Exported to {}", name), StatusSeverity::Success)
                }
                Err(e) if e != "Cancelled" => {
                    window.diagnostics.push(DiagnosticEvent::new(
                        LogLevel::Error,
                    ...
                    window.set_status(format!("Export error: {}", e), StatusSeverity::Error)
                }
                _ => Task::none(),
            },
        _ => Task::none(),
